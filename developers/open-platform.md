---
url: /developers/open-platform.md
...
## Obtaining client\_id and client\_secret

`client_id` and `client_secret` are the required credentials for integrating with Ham Connect:

* **client\_id**: The public identifier of the application, safe to use on the frontend
* **client\_secret**: The confidential credential of the application, **must only be used on the server side** — never expose it in frontend code, mobile app bundles, or public repositories
