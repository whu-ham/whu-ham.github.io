...
### 准备信息门户账号
图书馆座位预约系统通过信息门户（CAS）统一认证登录。你需要在Ham中设置好信息门户账号和密码。如果你忘记了信息门户账号密码，请前往信息门户网站重置。
...
Ham获取预约状态后，会自动将该事项添加到系统日历中，并在入馆前10分钟提醒。（iOS）

预约状态已适配灵动岛与LiveActivity，处于"已预约"状态的预约将会在通知栏中固定，并展示座位号、位置、时间等信息。（iOS）
